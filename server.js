...
// --- Groq Configuration ---
let groqClient;
const apiKey = process.env.GROQ_API_KEY; // Get key from environment

if (!apiKey) {
    console.error("FATAL ERROR: GROQ_API_KEY environment variable is not set. Cannot initialize Groq client.");
} else {
    try {
...
const expect = (value) => ({
    ...
    toEqual: (expected) => { if (JSON.stringify(value) !== JSON.stringify(expected)) throw new Error(\`Expected \${value} to equal \${expected}\`); },
...
app.post('/assert', async (req, res) => {
    ...
    if (!groqClient) {
        console.error('Error in /assert: groqClient was not initialized.');
...
// --- /analyze endpoint (For Refactor & Explain) ---
app.post('/analyze', async (req, res) => {
    const { code, mode } = req.body;
    if (!groqClient) {
        console.error('Error in /analyze: groqClient was not initialized.');
        return res.status(500).send({ error: 'AI model is not available. Check API Key.' });
    }
    if (!code || !mode) {
    ...
    try {
        ...
        // 2. Call Groq API
        console.log(`Sending prompt to Groq for mode: ${mode}`);
        const chatCompletion = await groqClient.chat.completions.create({
            messages: [{ role: 'user', content: formattedPrompt }],
            model: 'llama-3.3-70b-versatile',
            temperature: 0.5,
